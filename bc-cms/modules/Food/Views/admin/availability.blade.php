...
@section('content')
    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            <h1 class="title-bar">{{ __('Foods Availability Calendar') }}</h1>
        </div>
            ...
            <div class="panel-body">
                <div class="filter-div d-flex justify-content-between ">
                    ...
                    <div class="col-right">
                        @if ($rows->total() > 0)
                            <span
                                class="count-string">{{ __('Showing :from - :to of :total foods', ['from' => $rows->firstItem(), 'to' => $rows->lastItem(), 'total' => $rows->total()]) }}</span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        @else
            <div class="alert alert-warning">{{ __('No foods found') }}</div>
        @endif
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab', function(e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                foods: {
                    url: "{{ route('food.admin.availability.loadDates') }}",
                    extraParams: {
                        id: lastId,
                    }
                },
                ...
                foodClick: function(info) {
                    var form = Object.assign({}, info.food.extendedProps);
                    form.start_date = moment(info.food.start).format('YYYY-MM-DD');
                    form.end_date = moment(info.food.start).format('YYYY-MM-DD');
                    console.log(form);
                    formModal.show(form);
                },
                foodRender: function(info) {
                    $(info.el).find('.fc-title').html(info.food.title);
                    $(info.el).find('.fc-content').attr("data-html", "true").attr("title", info.food
                        .title).tooltip({
                        boundary: 'window'
                    })
                }
        ...
        $('.food-name:first-child a').trigger('click');
        ...
        formModal = new Vue({
            ...
            methods: {
                ...
                saveForm: function() {
                    ...
                    $.ajax({
                        url: '{{ route('food.admin.availability.store') }}',
                        data: this.form,
                        dataType: 'json',
                        method: 'post',
