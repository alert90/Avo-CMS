...
class AvailabilityController extends FrontendController{
    ...
    protected $indexView = 'Food::frontend.user.availability';
    ...
    public function index(Request $request){
        ...
        $page_title = __('Foods Availability');
    ...
    public function loadDates(Request $request){
        ...
        foreach ($period as $dt){
            $date = [
                ...
                'active'=>0,
                'textColor'=>'#2791fe',
                'price'=>(!empty($food->sale_price) and $food->sale_price > 0 and $food->sale_price < $food->price) ? $food->sale_price : $food->price,
            ];
            $date['start'] = $date['end'] = $dt->format('Y-m-d');
            if($food->default_state){
                $date['active'] = 1;
            }else{
                $date['title'] = $date['food'] = __('Blocked');
                $date['backgroundColor'] = 'orange';
                $date['borderColor'] = '#fe2727';
                $date['classNames'] = ['blocked-food'];
                $date['textColor'] = '#fe2727';
            }
            if ($food->ticket_types and $food->getBookingType() == "ticket") {
                $date['ticket_types'] = $food->ticket_types;
                $c_title = "";
                ...
                $date['title'] = $date['food'] = $c_title;
            }
            if ($food->getBookingType() == "time_slot") {
                if (!$is_single) {
                    $date['title'] = $date['food'] = format_money_main($date['price']);
                } else {
                    $date['title'] = $date['food'] = format_money($date['price']);
                }
                if ($time_slots = $food->getBookingTimeSlot()) {
        ...
        if(!empty($rows))
        {
            foreach ($rows as $row)
            {
                ...
                if ($row->ticket_types and $food->getBookingType() == "ticket") {
                    ...
                    $ticketData['title'] = $ticketData['food']  = $c_title;
                    $ticketData['ticket_types'] = $list_ticket_types;
                }

                if ($food->getBookingType() == "time_slot") {
                    if (!$is_single) {
                        $ticketData['title'] = $ticketData['food'] = format_money_main($row['price']);
                    } else {
                        $ticketData['title'] = $ticketData['food'] = format_money($row['price']);
                    }
                    $ticketData['price'] = $row['price'];
                }
                if(!$row->active)
                {
                    $ticketData['title'] = $row->food = __('Blocked');
                    $ticketData['backgroundColor'] = '#fe2727';
                    $ticketData['classNames'] = ['blocked-food'];
                    $ticketData['textColor'] = '#fe2727';
                    $ticketData['active'] = 0;
                }else{
                    $ticketData['classNames'] = ['active-food'];
                    $ticketData['active'] = 1;
                }
                $allDates[date('Y-m-d',strtotime($row->start_date))] = $ticketData;
            }
        }
        $bookings = $this->bookingClass::getAllBookingInRanges($food->id,$food->type,$request->query('start'),$request->query('end'));
        if(!empty($bookings))
        {
                ...
                foreach ($period as $dt){
                    ...
                    if(isset($allDates[$date])){
                        ...
                        if($isBook == false){
                            ...
                            $allDates[$date]['food'] = __('Full Book');
                            $allDates[$date]['title'] = __('Full Book');
                            $allDates[$date]['classNames'] = ['full-book-food'];
                        }
                    }
                }
            }
    ...
    public function store(Request $request){
        ...
        $food = $this->foodClass::find($request->input('target_id'));
        $target_id = $request->input('target_id');
        if(empty($food)){
            return $this->sendError(__('Food not found'));
        }
        if(!$this->hasPermission('food_manage_others')){
            if($food->author_id != Auth::id()){
        ...
        foreach ($period as $dt){
            ...
            $date = $this->foodDateClass::where('start_date',$dt->format('Y-m-d'))->where('target_id',$target_id)->first();
            if(empty($date)){
                $date = new $this->foodDateClass();
