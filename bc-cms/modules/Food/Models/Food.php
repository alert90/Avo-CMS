...
class Food extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bc_foods';
    public $type = 'food';
    public $checkout_booking_detail_file       = 'Food::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Food::frontend/booking/detail-modal';

    public $email_new_booking_file             = 'Food::emails.new_booking_detail';
    protected $translation_class = FoodTranslation::class;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->foodDateClass = FoodDate::class;
    ...
    /**
     * Get SEO for page list
     *
     * @return mixed
     */
    static public function getSeoMetaForPageList()
    {
        $meta['seo_title'] = __("Search for Food");
        if (!empty($title = setting_item_with_lang("food_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("food_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("food_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("food_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("food_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("food_page_list_seo_share");
    ...
    {
        return $this->hasMany($this->foodTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . env('FOOD_ROUTER_PREFIX', 'food') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    {
        return url(app_get_locale(false, false, '/') . env('FOOD_ROUTER_PREFIX', 'food') . "?" . http_build_query($param));
    }

    public function getEditUrl()
    {
        return url(route('food.admin.edit', ['id' => $this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        if (empty($total_tickets)) {
            return $this->sendError(__("Please select quantity!"));
        }

        //Buyer Fees for Admin
        $total_before_fees = $total;
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('food_booking_buyer_fees'))) {
    ...
    public function getDataAvailableBooking($start_date)
    {
        ...
        $foodDate = $this->foodDateClass::where('target_id', $this->id)->where('start_date', $start_date)->first();
        if (!empty($foodDate) and $foodDate->active == 0) {
            return [];
        }
        if (!empty($foodDate->ticket_types)) {
            $ticket_types = $foodDate->ticket_types;
        }
        ...
        foreach ($dataBooking as $booking) {
            ...
            if (!empty($bookingTicketTypes)) {
                foreach ($bookingTicketTypes as $bookingTicket) {
                    $numberBook = $bookingTicket['number'];
                    foreach ($ticket_types as &$ticket) {
                        if ($ticket['code'] == $bookingTicket['code']) {
                            $ticket['number'] =  $ticket['number'] - $numberBook;
                            if ($ticket['number'] < 0) {
                                $ticket['number'] = 0;
                            }
                        }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('food_booking_buyer_fees');
        if (!empty($list_fees)) {
    ...
    {
        return setting_item("food_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("food_review_approved", 0);
    }

    public function review_after_booking()
    {
        return setting_item("food_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("food_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("food_review_stats", []))) {
    ...
    {
        $food_id = $this->id;
        $list_score = Cache::rememberForever('review_' . $this->type . '_' . $food_id, function () use ($food_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $food_id)->where('object_model', "food")->where("status", "approved")->first();
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'object_author_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'food')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('food_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Foods", ['number' => $number]);
        }
        return __(":number Food", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        //Language
        $langs = $this->foodTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
    ...
    {
        return "ion-ios-restaurant";
    }

    public static function isEnable()
    {
        return setting_item('food_disable') == false;
    }
    ...
    {
        return (setting_item('food_deposit_enable') and setting_item('food_deposit_amount'));
    }
    public function getDepositAmount()
    {
        return setting_item('food_deposit_amount');
    }
    public function getDepositType()
    {
        return setting_item('food_deposit_type');
    }
    public function getDepositFomular()
    {
        return setting_item('food_deposit_fomular', 'default');
    }
    ...
    {
        if (!empty(setting_item('booking_enquiry_for_food'))) {
            return true;
        }
        return false;
    }
    ...
    {
        $check = setting_item('booking_enquiry_for_food');
        if (!empty($check) and setting_item('booking_enquiry_type_food') == "booking_and_enquiry") {
            return true;
        }
        return false;
    }
    public static function getBookingEnquiryType()
    {
        $check = setting_item('booking_enquiry_for_food');
        if (!empty($check)) {
            if (setting_item('booking_enquiry_type_food') == "only_enquiry") {
                return "enquiry";
            }
        }
        return "book";
    ...
    {
        $query = parent::query()->select("bc_foods.*");
        $query->where("bc_foods.status", "publish");
        if (!empty($location_id = $request['location_id'] ?? "")) {
            ...
            if (!empty($location)) {
                $query->join('bc_locations', function ($join) use ($location) {
                    $join->on('bc_locations.id', '=', 'bc_foods.location_id')
        ...
        if (!empty($price_range = $request['price_range'] ?? "")) {
            ...
            $raw_sql_min_max = "( (IFNULL(bc_foods.sale_price,0) > 0 and bc_foods.sale_price >= ? ) OR (IFNULL(bc_foods.sale_price,0) <= 0 and bc_foods.price >= ? ) )
                            AND ( (IFNULL(bc_foods.sale_price,0) > 0 and bc_foods.sale_price <= ? ) OR (IFNULL(bc_foods.sale_price,0) <= 0 and bc_foods.price <= ? ) )";
            $query->WhereRaw($raw_sql_min_max, [$pri_from, $pri_from, $pri_to, $pri_to]);
        }

        if ($term_id = $request['term_id'] ?? "") {
            $query->join('bc_food_term as tt1', function ($join) use ($term_id) {
                $join->on('tt1.target_id', "bc_foods.id");
                $join->where('tt1.term_id', $term_id);
            });
        }

        if (!empty($request['attrs'])) {
            $this->filterAttrs($query, $request['attrs'], 'bc_food_term');
        }
        ...
        if (!empty($service_name = $request["service_name"] ?? "")) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $query->leftJoin('bc_food_translations', function ($join) {
                    $join->on('bc_foods.id', '=', 'bc_food_translations.origin_id');
                });
                $query->where('bc_food_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $query->where('bc_foods.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        ...
        if (!empty($request['is_featured'])) {
            $query->where('bc_foods.is_featured', 1);
        }
        if (!empty($request['custom_ids']) and !empty($ids = array_filter($request['custom_ids']))) {
            $query->whereIn("bc_foods.id", $ids);
            $query->orderByRaw('FIELD (' . $query->qualifyColumn("id") . ', ' . implode(', ', $ids) . ') ASC');
        }
        $orderby = $request["orderby"] ?? "";
        switch ($orderby) {
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bc_foods.sale_price, 0 ) > 0 THEN bc_foods.sale_price ELSE bc_foods.price END AS tmp_min_price";
            ...
            default:
                if (!empty($request['order']) and !empty($request['order_by'])) {
                    $query->orderBy("bc_foods." . $request['order'], $request['order_by']);
                } else {
    ...
    public function locationBreadcrumbs()
    {
        $list = [];
        if ($this->location_id) {
            $location = $this->locationClass::find($this->location_id);
            if ($location) {
                $list[] = [
                    'name'  => $location->name,
                    'url'   => route('location.search', ['location_id' => $location->id]),
                    'class' => ''
                ];
            }
        }
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            $data['booking_fee'] = setting_item_array('food_booking_buyer_fees');
    ...
    {
        return "\Modules\Food\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("food")
            ]
        ];
    }
    ...
    {
        return setting_item('food_booking_type', 'ticket');
    }
    ...
    {
        $search_fields = setting_item_array('food_search_fields');
