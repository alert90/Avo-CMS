...
class Hotel extends Bookable
{
    ...
    public function getBookingData()
    {
        ...
        $booking_data = [
            ...
            'i18n'            => [
                ...
                "rooms"         => __('rooms'),
                "room"          => __('room'),
                "select_rooms"  => __("Please select rooms"),
            ],
        ...
        // Add room types for selection (similar to event ticket_types)
        $booking_data['room_types'] = $this->rooms->where('status', 'publish')->map(function ($room) {
            $translation = $room->translate();
            return [
                'id' => $room->id,
                'name' => $translation->title,
                'price' => $room->sale_price && $room->sale_price < $room->price ? $room->sale_price : $room->price,
                'max' => (int)$room->number,
                'min' => 0,
                'number' => 0, // for selection
                'display_price' => format_money($room->sale_price && $room->sale_price < $room->price ? $room->sale_price : $room->price),
                'adults' => $room->adults,
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            // Add rooms data for single hotel view (similar to event tickets)
            ...
            // Add room selection data similar to event ticket_types
            $data['room_types'] = $this->rooms->where('status', 'publish')->map(function ($room) {
                $translation = $room->translate();
                return [
                    'id' => $room->id,
                    'name' => $translation->title,
