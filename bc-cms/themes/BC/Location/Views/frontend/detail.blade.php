@extends('layouts.app')
@push('css')
    <link href="{{ asset('themes/bc/dist/frontend/module/location/css/location.css?_ver=' . config('app.asset_version')) }}"
        rel="stylesheet">
                            ...
                            <div class="col-12 col-lg-7">
                                <ul class="location-module-nav nav nav-pills justify-content-lg-end">
                                    ...
                                    @foreach ($types as $type => $moduleClass)
                                        @php
                                            $moduleInst = app($moduleClass);
                                            if (!$moduleInst::isEnable() or in_array($type, $not_in) == true or $moduleInst->hasLocationFeature === false) {
                                                continue;
                                            }
                                            $data[$type] = $moduleInst
                                                ...
                                                ->with(['translation','hasWishList'])
                                                ->take(8)
                                                ->get();

                                                // Set relation at runtime to prevent location relation query
                                                $data[$type]->each(function ($post) use ($row) {
                                                    $post->setRelation('location', $post);
                                                });
                                        @endphp
