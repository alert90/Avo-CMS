<form wire:submit.prevent="search"
    ...
    @php $tour_map_search_fields = setting_item_array('tour_map_search_fields');
        ...
        $tour_map_search_fields = array_values(
            \Illuminate\Support\Arr::sort($tour_map_search_fields, function ($value) {
    ...
    @if (!empty($tour_map_search_fields))
        @foreach ($tour_map_search_fields as $field)
            @switch($field['field'])
                @case ('location')
                    @include('Layout::livewire.search-map.fields.location', ['locationsList' => $this->tour_location,'location_search_style' => setting_item("tour_location_search_style")])
                @break

                @case ('category')
                    @include('Layout::livewire.search-map.fields.category', ['categoriesList' => $this->tour_category])
                @break

                @case ('attr')
                    @include('Layout::livewire.search-map.fields.attr', ['attrsList' => $this->attrsList])
                @break
    ...
    <script>
        $('.form-date-search',$wire.$el).each(function () {
            ...
            check_in_input.on('change', function () {
                $wire.setFilter('start', $(this).val(), false);
            });
            check_out_input.on('change', function () {
                $wire.setFilter('end', $(this).val(), false);
            });
        });
        $('[name="location_id"]',$wire.$el).on('change', function () {
            $wire.setFilter('location_id', $(this).val());
        });
        $('[name="cat_id[]"]',$wire.$el).on('change', function () {
            $wire.setFilter('cat_id', $(this).val());
        });

        $('.attr_id_hidden_input',$wire.$el).on('change', function () {
            $wire.setFilter('attrs', $(this).val());
        });
